...
; Video frames are full 176 dots across, 16 colours, 12 frames per second
;    stored as PCX, loaded in real time into the VDP to display as a bitmap.
;
...
open_ok:
	...
	ld	(pcx_handle),a	;store the file handle number for pcx file
;	call	inline_print
;	db		"Filename:",0
;	LD.LIL		HL,(IX+3)
;	call	print_HLU
...
;turn off cursor first = VDU 23,1,0
	...
	.ASSUME	adl=1
full_file_slurp:
; Returns DEU: Number of bytes read
;	ret
	ld	c,a	;handle
	ld.lil	hl,xpal_buffer
	ld.lil	de,0x060000		;grab the entire file up to 384k	;this is the file length of the .PCX header
...
;;	rst		10h
	ld		a,16	;4	;don't need 16 - breaks the VDP if we have a 4 colour mode
...
; Maybe kick this off after the next file slurp?
;Command 0: Play note
...
;	jr	@F	;test by skipping over this buffer routine
	...
	xor	a	;command 0
	rst	10h
	ld	hl,8800		;176 * 100 / 2 = . 160*90/2 = 7200.
;	ld	hl,7200	;6144	;64 bytes per row, 96 rows = 6144 bytes total. (pal_buffer+0x42)
...
scr_inner_chkwidth:
	...
	ex	de,hl
	ld	hl,8800		;176 * 100 / 2 = . 160*90/2 = 7200.
					;It's actually 64 bytes x72 rows = 4608 - how did this ever work?
					;64 bytes per row. 96 rows, so total is 160. (pal_buffer+0x42)
...
;;	rst	10h
; VDU 23, 0, &C3
; Swap/draw  the screen buffer (double-buffered modes only) or wait for VSYNC (all modes).
;;	ld	a,23
;;	rst	10h
;;	xor	a
;;	rst	10h
...
usage:	call	inline_print
	db	CR,LF,"PCXvideo utility for Agon by Shawn Sijnstra (c) 4-Feb-2026",CR,LF,CR,LF
	db	"Usage:",CR,LF
...
pcm_handle:	DS	1	;one byte for the audio PCM file handle
