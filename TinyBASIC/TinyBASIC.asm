...
;                   ALL WRONGS RESERVED
;
;                    Taking BChia's verion
;                  AND THEN PORTED TO AGON by
;                       Shawn Sijnstra 2026
;*************************************************************
...
;These should not be so... absolute
;Perhaps I need to preserve stack?
STACK:           EQU     07800H          ; STACK was 0FFFFH
OCSW:            EQU     02000H          ;SWITCH FOR OUTPUT
...
;These should not be so... absolute
TXTEND:          EQU     07700H          ;TEXT SAVE AREA ENDS
...
START:
;        LD SP,STACK                     ;*** COLD START *** - stack taken care of in wrapper.
;Don't change stack until we've done the Agon initialize.
        LD A,0FFH
        JP _start                       ;INIT
        DB "SS!"
...
_exec_name:             DB      "TinyBASIC.BIN", 0                ; The executable name, only used in argv

_start: PUSH.LIL        IY                      ; Preserve IY
;        PUSH            AF                      ; Preserve AF on the SPS stack before preserving the stack location

        LD              IY, 0                   ; Preserve SPS
        ADD             IY, SP
        PUSH.LIL        IY
        LD              SP, STACK               ; And set to 8000h, top of the MOS command area
...
;        POP.LIL         IX                      ; IX: argv
;        LD              B, 0                    ;  C: argc
        JP            INIT                   ; Start user code
;should return here but the stack is hard to manage and not sure it's bug free
;this only works because the entire app is in z80 mode, and the value of AF doesn't matter
_bail:
...
; AGON: For 'STOP', exit back to MOS
...
NEW:
        ...
        LD HL,TXTBGN
        LD (TXTUNF),HL
        CALL ENDCHK                     ;Duplicated below code because STOP
                                        ; now exits BASIC so we can't flow into it.
        JP RSTART
STOP:
        CALL ENDCHK                     ;*** STOP(CR) ***
;        JP RSTART
;        JP ENDCHK                       ;Return will go back to wrapper
        ld      hl,0                    ;return exit okay
        jp      _bail
RUN:
        CALL ENDCHK                     ;*** RUN(CR) ***
...
;        LD A,5CH                        ;AND ECHO A BACK-SLASH
        LD A,DEL                        ;Echo Delete to erase the character
        RST 10H
...
;        LD D,19H               ;Don't scroll or clear screen etc.
;PATLOP:
        CALL CRLF
;        DEC D
;        JR NZ,PATLOP
...
MSG1:   ;DB   12                         ;Agon CLS. ESC,"[2J",ESC,"[H"         ;SCREEN CLEAR
        DB   "Z80 TINY BASIC 2.0",CR       ;BOOT MESSAGE
MSG4:   DB   "WRITTEN BY DR LI-CHEN WANG, 1976",CR
MSG2:   DB   "PORTED BY DOUG GABBARD, 2017",CR
MSG3:   DB   "Agon Version Shawn Sijnstra, 2026",CR
...
; Let's use this to set up IX as the sysvars
;        MOSCALL mos_sysvars     ;get the sysvars location
        LD      A, mos_sysvars
        RST.LIS 08h
        RET
;-------------------------------------------------------------------------------
...
RX_RDY:
    ...
    ; This routine is for check if if a character is available over
    ...
    ; Z-flag set.
        ...
        ld      a,(IX+sysvar_vkeydown)          ;Virtual key state from FabGL (0=up, 1=down)
        or      a       ;only grab character if the key is down
        ret     z
        ld.lil  a,(IX+sysvar_keyascii)  ;fetch character in queue
        or      a
        ret     z
        ld      (IX+sysvar_keyascii),0
        ld      (IX+sysvar_vkeydown),0
        RET
...
;        ORG  08000H
;        ORG  0FF00H
        org  TXTEND     ;7F00h      ;pop it out of the way - maybe an issue here.
